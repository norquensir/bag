...
use Norquensir\Bag\Models\BoatSpot;
use Norquensir\Bag\Models\Building;
use Norquensir\Bag\Models\File;
use Norquensir\Bag\Models\Place;
use Norquensir\Bag\Models\PublicSpace;
use Norquensir\Bag\Models\ResidentialObject;
use Norquensir\Bag\Models\TrailerSpot;
...
class RunProcessJob implements ShouldQueue
{
    ...
    {
        $reader = XmlReader::fromFile(Storage::disk('bag')->path($file->path));
        ...
        if ($file->delete()) {
            Storage::disk('bag')->delete($file->path);
        }
    }
